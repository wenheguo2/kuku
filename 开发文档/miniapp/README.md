...
| 2026-07-29 | 字幕永不被裁根治：删句进度区 + 字号随句长自适应(用户：字多还被遮，建议取消第X/Y句留空间)：①删“第 X/Y 句”进度区，高度全给字幕(max-height 120→168PX)；②**字号随句长三挡**：≤18字 22PX、19-30字 19PX、>30字 16PX(小屏/大屏媒体查询挡同步三级)，任意句长保证放得下；h5 严格验证：极限 42 字句命中 .sm 挡 scrollHeight=clientHeight 句尾完整未被裁✓、短句大字挡✓、进度区已删✓；tsc+h5+weapp 三通(weapp 端用户手开 IDE 无自动化端口，样式同源待用户编译目验) | 用户：字数多还被遮挡+取消第X/Y句 |
| 2026-07-29 | **九项体验批改**(用户连发反馈)：①播钮白圓无三角—根因 weapp 渐变内 var(--color-primary) 失效致白底白图标→.cp/.hplay/.mini-v4 .mp/.ebtn.main 渐变色硬编码；②歌曲分类排序：幼儿/小孩儿/大孩儿/蒙学歌曲/诗词歌曲置顶(songCatalog CATEGORY_PRIORITY)；③成长宫格只展未交朋友的字/词(拉 /progress/:subject 过滤 stage>=1，按课序取最早未学；未登录回退前N课)；④今日歌单→💖你的播放列表(封面用“其他类型”分类图，点开=收藏页歌曲段)；⑤歌曲列表页右上🔁循环播放键(repeat-all+从第1首，h5实测列表循环模式生效)；⑥**退回首页无迷你栏根因：weapp 下 ScrollView 内 fixed 子元素被裁剪**→故事/歌曲/成长首页+song/list 四页 MiniPlayer/TabBar 移出 ScrollView(外层 View 包裹)；⑦故事续播队列短(≤4)时自动扩展为所在目录整列表(story/player expandQueueFromDir 拉父目录 entries 定位当前篇，h5实测从为你推荐点入→第21/190篇)；⑧播放器“列表”键改选单：▶连播我收藏的故事/回首页；收藏页支持 ?tab= 直达+故事段“连播全部”按钮+歌曲队列补真实 audioUrl/lrc/封面(修：原只有 id 无声)；⑨今日推荐按日轮换大IP+“换一个”钮(h5实测 西游记→封神演义→三国演义)，为你推荐窗口叠加按日偏移每天不同；tsc+h5+weapp 三通 | 用户：播钮无三角/分类排序/宫格未学优先/你的播放列表/循环键/无迷你栏/续播太少/收藏播放/推荐要动态 |
| 2026-07-29 | **搜索条目级改造 + 真题库接入 + 教学页三修**(用户连发：搜刘备无歌/搜的无字/去挑战没习题/单词不要田字格/立绘被裁/学习2背景不加载；并定原则 weapp/APP 优先 H5 第二挡)：①**搜索两级命中**—新脚本13 生成条目级索引(_search_story 10362条/1.2MB、_search_song 16073条/1.8MB，紧凑字段 t/p/s/c)；新 searchIndex.ts 懒拉+缓存；搜索页重写：story=学科+章回+单故事(直接入队播)、song=分类+单曲(入队播)、growth=学科+字/词条目(三学科全量词表，搜“的”直达教学播放器)，300ms 防抖+序号防竞态；h5 实测：刘备→4首单曲✓/的→识字字卡✓/哪吒→5单故事✓；②**server 真题库接入**—新 real-quiz.ts 读 production 习题 verify.json(prompt/options/answer)，普通+综合挑战真题优先无题回退合成；真题通过标准=答对≥75%；QuestionType 扩 6 真题型(前端 TYPE_LABEL 已有)；单测：识字/英语各4真题、拼音空回退✓；③教学页三修：切课/切挡先清旧态(学习2/3 背景立绘不加载根因，数据实测健全 location/角色全在库)+破图 onError 兑底隐藏；英语改“今日单词”宽卡无田字格(字号三挡 ≤6字40PX/7-9字30PX/>9字22PX 绝不裁字)；立绘改 flex 均分按人数自适应(修 4 人 32%×4 溢出被裁)；server+miniapp tsc 双通，server 已重启，h5+weapp 重建 | 用户：搜索要到单条目/挑战要真题/单词不用田字格/立绘自适应 |
| 2026-07-29 | **收集册+未遇见+具体字列表 + 迷你栏改右下小浮窗**(用户：收集册就数字不列未相识不知有多少/迷你栏垂圾)：①收集册每学科加“未遇见”数量(词库总数-已遇见)，四级均可点击→跳转词表页带 ?stage= 筛选，看到具体字列表；词表页支持 stage 路由参数作初始筛选；②迷你播放栏彻底换方案—不再底部大条(受 tabbar/ScrollView 影响太多)，改为★右下角小圆浮窗(88px 橙色圆钮，显示播/停图标，点击进播放器，右上 × 关闭)—不遮任何内容、不受 fixed 裁剪、tabbar 无关；去掉 onTab 参数；tsc+weapp 构建通过 | 用户：收集册加未遇见+具体字/迷你栏换个方案 |
